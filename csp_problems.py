...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            constraint_list.extend([AllDiffConstraint("Row",row)])
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            constraint_list.extend([AllDiffConstraint("Col",scope)])
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.extend([AllDiffConstraint("Subsquare",scope)])
