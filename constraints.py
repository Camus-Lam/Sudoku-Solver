...
class TableConstraint(Constraint):
    ...
    def hasSupport(self, var,val):
        '''check if var=val has an extension to an assignment of all variables in
           constraint's scope that satisfies the constraint. Only
...
class AllDiffConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        #   assignment
        #
        #Function #2 is only needed for efficiency
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        for val in assignments:
            if val in self._required:
                counter = counter + 1
        return self._lb <= counter <= self._ub
    ...
    def hasSupport(self, var, val):
        ...
        def vals_check(l):
            '''tests a list of assignments which are pairs (var,val)
               to see if they can satisfy the constraints'''
            vals = [val for (var, val) in l]
            counter = 0
            for num in vals:
                if num in self._required:
                    counter += 1
                if counter > self._ub:
                    return False
            return self._lb <= counter
        
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], vals_check)
        return x
